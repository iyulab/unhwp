"""Pytest harness for deterministic local unhwp binding verification."""
...
def _configure_native_library_path() -> None:
    if os.environ.get("UNHWP_LIB_PATH"):
        return

    built_library = _built_native_library_path()
    if built_library.exists():
        os.environ["UNHWP_LIB_PATH"] = str(built_library)
...
def pytest_report_header() -> str:
    configured_path = os.environ.get("UNHWP_LIB_PATH")
    if configured_path:
        return f"UNHWP_LIB_PATH={configured_path}"
    return f"UNHWP_LIB_PATH not set (expected build output: {_built_native_library_path()})"
