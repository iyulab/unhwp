...
mod tests {
    ...
    /// let `libunhwp-…` satisfy a search for `unhwp-…`, and the updater would replace the
    ...
    fn cli_archive_is_chosen_over_the_library_archive() {
        ...
        let release = vec![
            "libunhwp-windows-x86_64-v0.9.0.zip".to_string(),
            "unhwp-windows-x86_64-v0.9.0.zip".to_string(),
        ];

        assert_eq!(
            find_matching_asset(&release, &patterns).as_deref(),
            Some("unhwp-windows-x86_64-v0.9.0.zip")
        );
    }
    ...
    fn a_library_only_release_matches_nothing() {
        let patterns = get_asset_patterns(&windows_x86_64(), "0.9.0");
        let release = vec!["libunhwp-windows-x86_64-v0.9.0.zip".to_string()];
    ...
    fn naming_variants_resolve_in_preference_order() {
        ...
        let triple_only = vec!["unhwp-x86_64-pc-windows-msvc-0.9.0.zip".to_string()];
        assert_eq!(
            find_matching_asset(&triple_only, &patterns).as_deref(),
            Some("unhwp-x86_64-pc-windows-msvc-0.9.0.zip")
        );

        let several = vec![
            "unhwp-x86_64-pc-windows-msvc-v0.9.0.zip".to_string(),
            "unhwp-windows-x86_64-v0.9.0.zip".to_string(),
        ];
        assert_eq!(
    ...
    fn another_platforms_archive_is_not_matched() {
        let patterns = get_asset_patterns(&windows_x86_64(), "0.9.0");
        let release = vec![
            "unhwp-linux-x86_64-v0.9.0.tar.gz".to_string(),
            "unhwp-macos-aarch64-v0.9.0.tar.gz".to_string(),
        ];
    ...
    fn a_different_version_is_not_matched() {
        let patterns = get_asset_patterns(&windows_x86_64(), "0.9.0");
        let release = vec!["unhwp-windows-x86_64-v0.8.0.zip".to_string()];
